...
mod tests {
    ...
    fn adding_skips_edges_already_there_and_the_file_round_trips() {
        ...
        let dir = std::env::temp_dir().join(format!("dbt-edith-collin-{}", std::process::id()));
    ...
    fn discovery_reads_headers_and_orders_by_recency() {
        let dir = std::env::temp_dir().join(format!("dbt-edith-collin-disc-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn the_file_grows_only_with_new_snowflake_edges() {
        let dir = std::env::temp_dir().join(format!("dbt-edith-collin-file-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let path = dir.join("target").join("column_lineage.json");
        let edge = RawColEdge {
            from: "model.shop.stg_customers".into(),
