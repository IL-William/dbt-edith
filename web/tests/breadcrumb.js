...
print('--- path crumbs ---');
var deep = pathCrumbs('models/shop/staging/crm/stg_customers.yml');
check('one segment per folder and the file', labels(deep.map(function (c) { return { label: c.label }; })),
  ['models', 'shop', 'staging', 'crm', 'stg_customers.yml']);
check('the first segment lists the project root', deep[0].dir, '');
check('a segment lists the folder it sits in', deep[4].dir, 'models/shop/staging/crm');
check('and knows its own path, to mark itself current', deep[2].path, 'models/shop/staging');
...
// A manifest parsed on Windows used to reach the bar as one long segment whose
// menu listed the project root. The graph normalises it now; this holds the bar
// to the same answer either way.
var win = pathCrumbs('models\\shop\\crm\\stg__customers.sql');
check('a backslash path splits the same way', labels(win.map(function (c) { return { label: c.label }; })),
  ['models', 'shop', 'crm', 'stg__customers.sql']);
check('and its menus ask for a folder the server knows', win[3].dir, 'models/shop/crm');
check('a mixed path is still one path', pathCrumbs('models/shop\\crm/x.sql').length, 4);
...
print('\n--- yamlKey ---');
check('splits on a colon followed by a space', yamlKey('name: stg_customers'), { key: 'name', value: 'stg_customers' });
...
var PLAIN = ['satellites:', '  - sat_web__order', '  - "sat_shop__order"', '  - sat_app__order'].join('\n');
var pl = yamlOutline(PLAIN);
check('an item with no key of its own is labelled by its value', labels(pl),
  ['satellites', 'sat_web__order', 'sat_shop__order', 'sat_app__order']);
check('quotes are stripped, once', chain(pl, 2), ['satellites', 'sat_shop__order']);
